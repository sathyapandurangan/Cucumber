...
public class BaseClass1 {
	...
	public static String screenshort(String location) throws IOException {
		TakesScreenshot ts = (TakesScreenshot) driver;
		String Timestamp = new SimpleDateFormat("yyyMMdd_HHmmss").format(new Date(0));
		File source = ts.getScreenshotAs(OutputType.FILE);
		File destination = new File(location + "_" + Timestamp + ".png");
	...
	// 29.Explicit wait
	public static void Explicit(WebElement element, Duration x) {
	...
	public static String fullScreenShot(String location) throws IOException {
		ru.yandex.qatools.ashot.Screenshot take = new AShot().shootingStrategy(ShootingStrategies.viewportPasting(1000))
				.takeScreenshot(driver);
		ImageIO.write(take.getImage(), "PNG", new File(location));
		return location;
	}

	// 32.extendReportStart
	public static void extendReportStart(String location) throws IOException {
		ExtendReports = new ExtentReports();
		file = new File(location);
		ExtentSparkReporter SpeakerReporter = new ExtentSparkReporter(file);
		ExtendReports.attachReporter(SpeakerReporter);
